...
export default function GameFeature({ game }: { game: Game }) {
    ...
    return (
        <article className="mx-auto grid max-w-5xl overflow-hidden rounded bg-bg-secondary md:grid-cols-[minmax(240px,0.62fr)_minmax(0,1.38fr)]">
            <div className="order-2 flex flex-col justify-between gap-4 bg-bg-secondary/80 p-4 md:order-1 md:h-96 md:p-5">
                <div className="flex flex-col gap-3">
                    <Link href={`/game/${game.slug}`} className="relative mb-1 block aspect-video overflow-hidden rounded bg-bg md:hidden">
                        {heroSrc ? (
                            <Image
                                src={heroSrc}
                        ...
                        {coverSrc ? (
                            <div className="absolute bottom-3 right-3 aspect-3/4 w-16 overflow-hidden rounded border border-border bg-bg-secondary">
                                <Image
                                    src={coverSrc}
                                    alt={`${game.name ?? "Game"} cover`}
                                    fill
