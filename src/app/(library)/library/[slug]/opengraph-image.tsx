...
export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
	...
	const publicLibrary = isPublic && library ? library.name : "Private library";
	const publicDescription = isPublic && library ? `Browse ${library.name} on TrackGames.` : "This TrackGames library is private.";
	const title = library ? publicLibrary : "Library not found";
	const description = library ? publicDescription : "The requested library could not be found.";
	const gameCount =
		isPublic && library
			? await db.userGameEntry.count({
					where: {
