				...
				return (
					<article key={entry.id} className={joinClass("overflow-hidden rounded border transition-colors", expanded ? "border-primary/50" : "border-border")}>
						<button
							...
							className="flex w-full items-center gap-4 bg-bg-secondary/40 p-5 text-left transition-colors hover:bg-bg-secondary/70"
						>
							<div className="flex min-w-0 flex-1 flex-col gap-1">
								...
								<div className="flex items-center gap-2 text-xs text-text-faint">
									...
									{entry.summary && !expanded && <span className="truncate text-text-muted">— {entry.summary}</span>}
								</div>
							</div>
						...
						{expanded && (
							<div className="flex flex-col gap-4 border-t border-border p-5">
								{entry.summary && <p className="text-sm text-text-muted">{entry.summary}</p>}
								{entry.content.trim() ? (
