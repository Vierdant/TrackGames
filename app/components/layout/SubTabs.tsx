...
export default function SubTabs({ tabs, active, setter, viewAll, compact, children }: SubTabsProps) {
	if (!tabs.length) {
		return null;
	}
	...
	return (
		<div className="min-w-0">
			...
			<nav
				className={`mb-5 min-w-0 flex-row items-center gap-2 border-b border-border ${compact ? "hidden md:flex" : "flex"}`}
				aria-label="Tabs"
			>
				{tabs.map((tab) => {
					const isActive = tab.id === active;
