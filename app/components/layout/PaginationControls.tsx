...
export default function PaginationControls({ page, pageCount, href, onPageChange }: { page: number; pageCount: number; href?: (page: number) => string; onPageChange?: (page: number) => void }) {
    ...
    function renderPageButton(value: number, label: string, children: ReactNode, disabled?: boolean) {
        const className = `grid size-9 place-items-center rounded border text-sm font-bold transition-colors ${safePage === value
            ? "border-primary bg-primary text-text"
            : "border-border text-text-muted hover:border-primary hover:text-primary"} ${disabled ? "pointer-events-none opacity-50" : ""}`;
        ...
        return (
            <button type="button" onClick={() => goToPage(value)} disabled={disabled} className={`${className} cursor-pointer disabled:cursor-default`} aria-label={label} aria-current={safePage === value ? "page" : undefined}>
                {children}
            </button>
        );
    }
        ...
        <div className="flex flex-row items-center justify-center gap-3">
            ...
            <div className="hidden flex-row items-center gap-2 sm:flex">
                {pageNumbers.map((number) => (
                    <span key={number}>
                        {renderPageButton(number, `Page ${number}`, number)}
                    </span>
                ))}
                {safePageCount > 7 && (
                    <>
