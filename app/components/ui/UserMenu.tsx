...
import { ArrowLeft, ArrowRight, Bell, Book, CircleQuestionMark, LogOut, Menu, Settings, User as UserIcon } from "lucide-react";
    ...
    return (
        <div className="relative">
            <button
                ...
                aria-label={unread ? "Open user menu, unread notifications" : isOpen ? "Close user menu" : "Open user menu"}
            ...
            <MenuPanel
                ...
                showClose={false}
                anchorRef={buttonRef}
                className={isSmallScreen ? `fixed inset-0 bg-overlay transition-opacity ${isOpen ? "opacity-100" : "pointer-events-none opacity-0"}` : undefined}
                panelClassName={isSmallScreen
                    ? `!fixed !bottom-0 !right-0 !top-0 !mt-0 flex !w-[min(21rem,calc(100vw-1.5rem))] origin-right flex-col border-l border-border !p-0 text-text ${isOpen ? "animate-menu-drawer-in" : "animate-menu-drawer-out"}`
