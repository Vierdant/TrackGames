...
export default function FollowerPreviewPanel({ title, count, profiles }: {
    title: string; count: number; profiles: {
        name: string;
        image: string | null;
    }[]
}) {
    ...
    return (
        <div className="w-full max-w-4xl rounded bg-bg-secondary/80 p-4">
            <div className="mb-3 flex items-center justify-between gap-4 border-b border-border pb-2">
                <h2 className="text-sm">{title} - {count}</h2>
            </div>
            ...
            <div className="grid w-full grid-cols-4 gap-2 rounded p-2">
                {visible.length > 0 ? (
                    <>
                        ...
                        {overflow > 0 && (
                            <div className="grid size-10 place-items-center rounded-full border border-border bg-bg text-xs font-bold text-text-muted">
                                +{overflow}
                            </div>
                        )}
                    </>
